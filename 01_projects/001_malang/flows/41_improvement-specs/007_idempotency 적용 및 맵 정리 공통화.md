...
- `routeRealtimeEvent + eventRouter`를 도입해 주요 이벤트를 라우터 테이블 기반으로 분리했다.
- `response.done` 처리를 목적별 핸들러(`slot_detection`, `text_enrichment`, `fsm_reply`)로 분리했다.
...
## 다음 작업
- 검증 체크 실행 및 체크리스트 반영(중복 실행/역순 이벤트/수동 이벤트)
- 완료된 4단계 결과 점검(전사 가드 공통화, action 유틸화, 로그 키 표준화)
- 서버 idempotency 스펙 항목은 최우선 백로그로 유지
